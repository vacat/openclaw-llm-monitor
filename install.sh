...
show_help() {
    ...
    echo ""
    echo "通用选项:"
    echo "  -a, --agents         指定 agent(s)，逗号分隔或 \"all\""
    echo "  -d, --dir            指定监控目录 (仅 monitor)"
    echo "  --date YYYY-MM-DD    指定日期 (仅 stats)"
    echo "  --today              查看今日统计 (仅 stats)"
    echo "  --by-agent           按 agent 分组显示 (仅 stats)"
    echo ""
    echo "示例:"
    ...
    echo "  openclaw-monitor stats -a main              # 查看单个 agent"
    echo "  openclaw-monitor stats -a all               # 查看所有 agent"
    echo "  openclaw-monitor stats -a all --by-agent    # 按 agent 分组"
...
show_usage() {
    ...
    echo ""
    echo "  2. 监控单个 agent:"
    echo -e "     ${YELLOW}openclaw-monitor monitor -a main${NC}"
    echo ""
    echo "  3. 监控多个 agent:"
    ...
    echo ""
    echo "  6. 查看所有 agent 统计:"
    echo -e "     ${YELLOW}openclaw-monitor stats -a all${NC}"
    echo ""
    echo "  7. 查看帮助:"
